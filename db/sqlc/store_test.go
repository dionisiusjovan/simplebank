...
func TestTransferTx(t *testing.T) {
	...
	for i := 0; i < n; i++ {
		...
		require.NotZero(t, fromAccount.Currency)
		require.NotZero(t, fromAccount.CreatedAt)

		_, err = store.GetAccount(context.Background(), transfer.FromAccountID)
		require.NoError(t, err)
		...
		require.NotZero(t, toAccount.Currency)
		require.NotZero(t, toAccount.CreatedAt)

		_, err = store.GetAccount(context.Background(), transfer.ToAccountID)
		require.NoError(t, err)
