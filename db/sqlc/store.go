...
func (store *Store) execTx(ctx context.Context, fn func(*Queries) error) error {
	tx, err := store.db.BeginTx(ctx, nil) //using default val
	if err != nil {
		return err
	}
...
//Creates a transfer record + Add account entries + Update accounts' balance within single db tx
func (store *Store) TransferTx(ctx context.Context, arg TransferTxParams) (TransferTxResult, error) {
	...
	err := store.execTx(ctx, func(q *Queries) error {
		...
		// fmt.Println(txName, "create entry src")
		...
		// fmt.Println(txName, "create entry dest")
